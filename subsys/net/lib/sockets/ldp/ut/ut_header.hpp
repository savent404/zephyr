...
struct mock_work_queue: public systech::cif::work_queue_if {
	MOCK_METHOD(id, enqueue,
		    (void (*fn)(void *, void *), void *arg1, void *arg2, uint32_t delay),
		    (override));
...
struct simu_mcb: public systech::cif::mcb_if {
	...
	virtual void clr_status(uint32_t bits) override
	{
		status_[sid_] &= ~bits;
	}
	virtual bool has_rx(uint8_t port) override
	{
		return data_ready_[sid_][port];
	}

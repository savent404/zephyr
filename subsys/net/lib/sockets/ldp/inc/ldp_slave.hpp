...
template <typename T_cache> struct ldp_slave : public ldp_basic {
  ...
  virtual int create(bool is_async, ldp_config *config) {
    ...
    if (std::find_if(conns_.begin(), conns_.end(),
                     [config](const conn_info_ptr &cip) {
  ...
  virtual int recv(conn c, uint8_t *buf, uint16_t len) {
    ...
    if (!(*it)->is_async) {
      ...
      tx_hdr =
          reinterpret_cast<volatile ldp_a_header *>(mcb_->get_tx_buf(port));
  ...
  virtual uint32_t get_extra_error(conn c) { return 0; }

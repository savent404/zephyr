...
static void reg_write(uint8_t size, uint32_t data, mm_reg_t addr, uint32_t off)
{
	...
	/*
	 * Workaround for low speed SPI devices.
	 * The delay is required for the SPI controller to stabilize the clock
	 * and data lines after the write operation.
	 */
	k_busy_wait(1);
#endif
	switch (size) {
	case 8:
		sys_write8(data, addr + off);
		break;
	case 16:
		sys_write16(data, addr + off);
		break;
	case 32:
...
#define SPI_DW_CLK_DIVIDER(clock_freq, ssi_clk_hz) ((clock_freq / ssi_clk_hz) & 0xFFFF)
...
/* Error interrupt */
#define DW_SPI_ISR_ERRORS_MASK                                                                     \
	(DW_SPI_ISR_TXOIS | DW_SPI_ISR_RXUIS | DW_SPI_ISR_RXOIS | DW_SPI_ISR_MSTIS)
/* ICR Bit */
#define DW_SPI_SR_ICR_BIT (0)

/* Interrupt mask (IMR) */
#define DW_SPI_IMR_MASK (0x0)
#define DW_SPI_IMR_UNMASK                                                                          \
	(DW_SPI_IMR_TXEIM | DW_SPI_IMR_TXOIM | DW_SPI_IMR_RXUIM | DW_SPI_IMR_RXOIM |               \
	 DW_SPI_IMR_RXFIM)
#define DW_SPI_IMR_MASK_TX (~(DW_SPI_IMR_TXEIM | DW_SPI_IMR_TXOIM))
#define DW_SPI_IMR_MASK_RX (~(DW_SPI_IMR_RXUIM | DW_SPI_IMR_RXOIM | DW_SPI_IMR_RXFIM))

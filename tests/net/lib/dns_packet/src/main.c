 ...
 * Recursion desired
 */
static uint8_t query_ipv4[] = {0xda, 0x0f, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
			       0x00, 0x00, 0x03, 0x77, 0x77, 0x77, 0x0d, 0x7a, 0x65, 0x70,
			       0x68, 0x79, 0x72, 0x70, 0x72, 0x6f, 0x6a, 0x65, 0x63, 0x74,
...
static uint8_t query_mdns[] = {
	0xda, 0x0f, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x7a, 0x65,
	0x70, 0x68, 0x79, 0x72, 0x05, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x00, 0x00, 0x01, 0x00, 0x01,
};
...
static int eval_query(const char *dname, uint16_t tid, enum dns_rr_type type, uint8_t *expected,
		      uint16_t expected_len)
{
	...
	rc = dns_msg_pack_query(dns_buf, &dns_buf_len, MAX_BUF_SIZE, qname, qname_len, tid, type);
	if (rc != 0) {
		goto lb_exit;
	}
...
ZTEST(dns_packet, test_dns_query)
{
	...
	rc = eval_query(DNAME1, tid1, DNS_RR_TYPE_A, query_ipv4, sizeof(query_ipv4));
	zassert_equal(rc, 0, "Query test failed for domain: " DNAME1);

	rc = eval_query(NULL, tid1, DNS_RR_TYPE_A, query_ipv4, sizeof(query_ipv4));
	zassert_not_equal(rc, 0, "Query test with invalid domain name failed");

	rc = eval_query(DNAME1, tid1, DNS_RR_TYPE_AAAA, query_ipv4, sizeof(query_ipv4));
	zassert_not_equal(rc, 0, "Query test for IPv4 with RR type AAAA failed");

	rc = eval_query(DNAME1, tid1 + 1, DNS_RR_TYPE_A, query_ipv4, sizeof(query_ipv4));
 ...
 * RData: 140.211.169.8
 */
static uint8_t resp_ipv4[] = {0xb0, 0x41, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00,
			      0x00, 0x03, 0x77, 0x77, 0x77, 0x0d, 0x7a, 0x65, 0x70, 0x68, 0x79,
			      0x72, 0x70, 0x72, 0x6f, 0x6a, 0x65, 0x63, 0x74, 0x03, 0x6f, 0x72,
			      0x67, 0x00, 0x00, 0x01, 0x00, 0x01, 0xc0, 0x0c, 0x00, 0x01, 0x00,
			      0x01, 0x00, 0x00, 0x0b, 0xd4, 0x00, 0x04, 0x8c, 0xd3, 0xa9, 0x08};

static const uint8_t resp_ipv4_addr[] = {140, 211, 169, 8};

ZTEST(dns_packet, test_dns_response)
{
	struct dns_response_test test = {.dname = DNAME1,
					 .res = resp_ipv4,
	...
	zassert_equal(rc, 0, "Response test failed for domain: " DNAME1 " at line %d", -rc);

	/* Test also using dns_unpack_answer() API */
	memcpy(&test2, &test, sizeof(test2));
	rc = eval_response1(&test2, true);
	zassert_equal(rc, 0, "Response test 2 failed for domain: " DNAME1 " at line %d", -rc);
}
 ...
 */
char answer_ipv4[] = {0x21, 0x21, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00,
		      0x00, 0x03, 0x77, 0x77, 0x77, 0x09, 0x77, 0x69, 0x72, 0x65, 0x73,
		      0x68, 0x61, 0x72, 0x6b, 0x03, 0x6f, 0x72, 0x67, 0x00, 0x00, 0x01,
		      0x00, 0x01, 0x01, 0x61, 0xc0, 0x0c, 0x00, 0x01, 0x00, 0x01, 0x00,
		      0x00, 0x02, 0x58, 0x00, 0x04, 0xae, 0x89, 0x2a, 0x41};
...
{
	struct dns_response_test test1 = {.dname = DNAME2,
					  .res = answer_ipv4,
	...
	zassert_equal(rc, 0, "Response test 2 failed for domain: " DNAME2 " at line %d", -rc);
}
...
ZTEST(dns_packet, test_mdns_query)
{
	...
	rc = eval_query(ZEPHYR_LOCAL, tid1, DNS_RR_TYPE_A, query_mdns, sizeof(query_mdns));
...
static uint8_t resp_ipv6[] = {
	0xf2, 0xb6, 0x80, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x06, 0x7a,
	0x65, 0x70, 0x68, 0x79, 0x72, 0x05, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x00, 0x00, 0x1c,
	0x00, 0x01, 0x06, 0x7a, 0x65, 0x70, 0x68, 0x79, 0x72, 0x05, 0x6c, 0x6f, 0x63, 0x61,
	0x6c, 0x00, 0x00, 0x1c, 0x00, 0x01, 0x00, 0x00, 0x00, 0x1e, 0x00, 0x10, 0xfe, 0x80,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x5e, 0xff, 0xfe, 0x00, 0x53, 0x37,
};

static const uint8_t resp_ipv6_addr[] = {0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
ZTEST(dns_packet, test_mdns_response)
{
	...
	rc = eval_response1(&test1, true);
	zassert_equal(rc, 0, "Response test failed for domain: " ZEPHYR_LOCAL " at line %d", -rc);
}

static uint8_t resp_truncated_response_ipv4_1[] = {
	/* DNS msg header (12 bytes) */
	0xb0, 0x41, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,

	/* Query string (www.zephyrproject.org) */
	0x03, 0x77, 0x77, 0x77, 0x0d, 0x7a, 0x65, 0x70, 0x68, 0x79, 0x72, 0x70, 0x72, 0x6f, 0x6a,
	0x65, 0x63, 0x74, 0x03, 0x6f, 0x72, 0x67, 0x00,
...
static uint8_t resp_valid_response_ipv4_8[] = {
	/* DNS msg header (12 bytes) */
	0xb0, 0x41, 0x81, 0x80, 0x00, 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00,
...
static uint8_t resp_valid_response_ipv4_11[] = {
	/* DNS msg header (12 bytes) */
	0x74, 0xe1, 0x81, 0x80, 0x00, 0x01, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00,

	/* Query string */
	0x0e, 0x77, 0x65, 0x73, 0x74, 0x75, 0x73, 0x32, 0x2d, 0x70, 0x72, 0x6f, 0x64, 0x2d, 0x32,
	0x0d, 0x6e, 0x6f, 0x74, 0x69, 0x66, 0x69, 0x63, 0x61, 0x74, 0x69, 0x6f, 0x6e, 0x73, 0x05,
	0x74, 0x65, 0x61, 0x6d, 0x73, 0x09, 0x6d, 0x69, 0x63, 0x72, 0x6f, 0x73, 0x6f, 0x66, 0x74,
	...
	/* Data */
	0x11, 0x77, 0x65, 0x73, 0x74, 0x75, 0x73, 0x32, 0x63, 0x6e, 0x73, 0x2d, 0x70, 0x72, 0x6f,
	0x64, 0x2d, 0x32, 0x0e, 0x74, 0x72, 0x61, 0x66, 0x66, 0x69, 0x63, 0x6d, 0x61, 0x6e, 0x61,
	...
	/* Data */
	0x14, 0x77, 0x65, 0x73, 0x74, 0x75, 0x73, 0x32, 0x63, 0x6e, 0x73, 0x2d, 0x70, 0x72, 0x6f,
	0x64, 0x2d, 0x32, 0x2d, 0x31, 0x36, 0x07, 0x77, 0x65, 0x73, 0x74, 0x75, 0x73, 0x32, 0x08,
	0x63, 0x6c, 0x6f, 0x75, 0x64, 0x61, 0x70, 0x70, 0x05, 0x61, 0x7a, 0x75, 0x72, 0x65, 0xc0,
	0x39,
...
static void resolve_cb(enum dns_resolve_status status, struct dns_addrinfo *info, void *user_data)
{
	ARG_UNUSED(status);
	ARG_UNUSED(info);
...
static void setup_dns_context(struct dns_resolve_context *ctx, int idx, uint16_t dns_id,
...
static void run_dns_malformed_response(const char *test_case, uint8_t *buf, size_t len)
{
	/* Query is used to calculate the hash, it contains the
	 * labels + query type
	 */
	static const uint8_t query[] = {/* Labels */
					0x03, 0x77, 0x77, 0x77, 0x0d, 0x7a, 0x65, 0x70, 0x68, 0x79,
					0x72, 0x70, 0x72, 0x6f, 0x6a, 0x65, 0x63, 0x74, 0x03, 0x6f,
					0x72, 0x67, 0x00,
					/* Query type */
					0x00, 0x01};
	 ...
	 */
	if (len > 12) {
		setup_dns_context(&dns_ctx, 0, dns_id, query, sizeof(query), DNS_QUERY_TYPE_A);
	}

	ret = dns_validate_msg(&dns_ctx, &dns_msg, &dns_id, &query_idx, NULL, &query_hash);
	zassert_not_equal(ret, DNS_EAI_ALLDONE, "[%s] DNS message was valid (%d)", test_case, ret);
}

static void run_dns_valid_response(const char *test_case, uint8_t *buf, size_t len)
{
	struct dns_msg_t dns_msg = {0};
	uint16_t dns_id = 0;
	int query_idx = -1;
	...
	ret = dns_validate_msg(&dns_ctx, &dns_msg, &dns_id, &query_idx, NULL, &query_hash);
	zassert_equal(ret, DNS_EAI_ALLDONE, "[%s] DNS message failed (%d)", test_case, ret);
}
...
NET_BUF_POOL_DEFINE(dns_qname_pool, DNS_RESOLVER_BUF_CTR, DNS_MAX_NAME_LEN, 0, NULL);

static void run_dns_valid_cname_response(const char *test_case, uint8_t *buf, size_t len,
					 int expected_ret)
{
	...
	ret = dns_validate_msg(&dns_ctx, &dns_msg, &dns_id, &query_idx, dns_cname, &query_hash);
	zassert_equal(ret, expected_ret, "[%s] DNS message failed (%d)", test_case, ret);
}

#define RUN_VALID_TEST(test_name) run_dns_valid_response(#test_name, test_name, sizeof(test_name))

#define RUN_VALID_CNAME_TEST(test_name, expected_ret)                                              \
	run_dns_valid_cname_response(#test_name, test_name, sizeof(test_name), expected_ret)
...
ZTEST(dns_packet, test_dns_id_len)
{
	...
	ret = dns_validate_msg(&dns_ctx, &dns_msg, &dns_id, &query_idx, NULL, &query_hash);
	zassert_equal(ret, DNS_EAI_FAIL, "DNS message length check failed (%d)", ret);
}
...
static uint8_t invalid_answer_resp_ipv4[18] = {
	/* DNS msg header (12 bytes) */
	0x01, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
...
static uint8_t recursive_query_resp_ipv4[] = {
	/* DNS msg header (12 bytes) */
	0x74, 0xe1, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
...
ZTEST(dns_packet, test_dns_recursive_query)
{
	...
	zassert_true(ret == DNS_EAI_SYSTEM && errno == ELOOP,
		     "[%s] DNS message was valid (%d / %d)", "recursive rsp", ret, errno);
...
static uint8_t invalid_compression_response_ipv4[] = {
	...
	/* Answer 1 */
	0xb0,
	0x0c, /* <--- invalid compression pointer */
...
ZTEST(dns_packet, test_dns_invalid_compress_bits)
{
	...
	zassert_true(ret == DNS_EAI_SYSTEM && errno == EINVAL,
		     "[%s] DNS message was valid (%d / %d)", "invalid compression rsp", ret, errno);
